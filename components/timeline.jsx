  ...
  "flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 bg-background text-xs font-medium",
  {
    variants: {
      status: {
        ...
        completed: "border-green-500 bg-green-500 text-primary-foreground",
        processing:
          "border-blue-500 bg-blue-500 text-primary-foreground animate-pulse",
        pending: "border-muted-foreground/30 text-muted-foreground",
        failed: "border-red-500 bg-red-500 text-destructive-foreground",
      },
    },
    defaultVariants: {
      status: "default",
      ...
      {items.map((item, index) => (
        ...
        >
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-2">
            {/* Timestamp - Top */}
            {item.header && (
              <div className="text-xs text-muted-foreground">{item.header}</div>
            )}
            {showTimestamps &&
              timestampPosition === "top" &&
              item.timestamp && (
            ...
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-normal leading-tight">{item.title}</h3>
              {showTimestamps &&
