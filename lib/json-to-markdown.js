...
function extractCitation(valueObj) {
  ...
  return {
    ...
    source: grounding?.source ?? valueObj?.source ?? null,
  };
}
...
function extractCitationFromRow(rowObj) {
  ...
  return {
    pageIndex: Math.max(0, Number(pageNo) - 1),
    bbox: bbox.map((n) => Number(n)),
    source: rowObj?.grounding?.source ?? rowObj?.source ?? null,
  };
}
...
function wrapWithCitationSpan(htmlSafeText, citation) {
  ...
  const title = citation?.source ? `Citation: ${citation.source}` : "Citation";
  return `<span class="citation-value" data-citation="${payload}" title="${escapeHtml(
    title
  )}">${htmlSafeText}</span>`;
}
