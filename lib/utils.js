...
export function autoDetectAndNormalize(bbox, imageWidth, imageHeight) {
  if (!bbox) return null;

  // Accept either [l,t,r,b] or { l,t,r,b } (or { left, top, right, bottom })
  ...
  // Case 1: already normalized [0..1] (like sample-response.json)
  if (max <= 1.000001) {
    return nums.map((n) => clamp01(n));
  }

  if (max <= 100.000001 && max > 1.000001) {
    return nums.map((n) => clamp01(n / 100));
  }
  ...
  if (
    ...
    return [clamp01(l / w), clamp01(t / h), clamp01(r / w), clamp01(b / h)];
  }
